...
/**
 * REST controller for managing {@link com.virality.dishly.domain.Address}.
 */
@RestController
...
public class AddressResource {
            ...
            .flatMap(exists -> {
                ...
                Mono<Address> result = addressRepository
                    ...
                    .map(existingAddress -> {
                        ...
                        if (address.getYmapY() != null) {
                            existingAddress.setYmapY(address.getYmapY());
                        }
                        if (address.getYmapX() != null) {
                            existingAddress.setYmapX(address.getYmapX());
                        }
    ...
    public Mono<List<Address>> getAllAddresses() {
        LOG.debug("REST request to get all Addresses");
        return addressRepository.findAll().collectList();
    }
    ...
    public Flux<Address> getAllAddressesAsStream() {
        LOG.debug("REST request to get all Addresses as a stream");
     ...
     */
    @GetMapping("/{id}")
    public Mono<ResponseEntity<Address>> getAddress(@PathVariable("id") Long id) {
